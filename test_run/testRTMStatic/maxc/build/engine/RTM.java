...
super(parameters);
OffsetExpr nx = stream.makeOffsetParam("nx", min_nx / Par, max_nx / Par);
OffsetExpr nxy = stream.makeOffsetParam("nxy", dim_y * nx, dim_y * nx);
...
KArray<HWVar> burst_p = io.input("burst_p",  new KArrayType<HWVar>(hwFloat(8, 24),Par)).cast(new KArrayType<HWVar>(hwFloat(8, 12),Par));
KArray<HWVar> burst_dvv = io.input("burst_dvv",  new KArrayType<HWVar>(hwFloat(8, 24),Par));
KArray<HWVar> burst_pp = io.input("burst_pp",  new KArrayType<HWVar>(hwFloat(8, 24),Par));
KArray<HWVar> burst_source = io.input("burst_source",  new KArrayType<HWVar>(hwFloat(8, 24),Par));
KArray<HWVar> output_pp = (new KArrayType<HWVar>(hwFloat(8, 12),Par)).newInstance(this);
KArray<HWVar> ker_p = (new KArrayType<HWVar>(hwFloat(8, 24),Par)).newInstance(this);

Count.Params param0 = control.count.makeParams(32).withMax(n1).withInc(Par);
Counter i1_counter = control.count.makeCounter(param0);
HWVar i1 = i1_counter.getCount();
Count.Params param1 = control.count.makeParams(32).withMax(n2).withInc(1).withEnable(i1_counter.getWrap());
Counter i2_counter = control.count.makeCounter(param1);
HWVar i2 = i2_counter.getCount();
Count.Params param2 = control.count.makeParams(32).withMax(n3).withInc(1).withEnable(i2_counter.getWrap());
Counter i3_counter = control.count.makeCounter(param2);
HWVar i3 = i3_counter.getCount();
Count.Params param3 = control.count.makeParams(10).withMax(1000).withInc(1).withEnable(i3_counter.getWrap());
Counter i4_counter = control.count.makeCounter(param3);
...
inter[i][0] = up[i].eq(0)?pp_i[i]:result[i][0];
}
for (int j = 1;j < Mul;j++) {
for (int i = 0;i < Par;i++) {
int k = (-6 / Par);
...
inter[i][j] = up[i].eq(0)?cur[j - 1][6 + i][5][5]:result[i][j];
}
}
optimization.popDSPFactor();
for (int i = 0;i < Par;i++) {
ker_p[i] <== inter[i][Mul - 1];
output_pp[i] <== cur[Mul - 1][6 + i][5][5];
}

io.output("output_pp", output_pp.cast(hwFloat(8, 24)), new KArrayType<HWVar> (hwFloat(8, 24), Par));
io.output("ker_p", ker_p, new KArrayType<HWVar> (hwFloat(8, 24), Par));
}}