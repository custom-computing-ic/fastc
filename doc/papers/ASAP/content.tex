...
\XXX{Why??}: a design is composed of one or more computational kernels
...
Figure \ref{fig:maxc-1dconv} shows an example kernel which implements
...
\item \emph{regular C constructs} are used as much as possible to make
  ...
  conventions for implementing kernels.\XXX{Unlike StreamsC which uses
    the comment style to define kernel bodies etc.}; We also
...
% in(), out(), or kerneldef( “name”, in(), out() ) { /** Kernel Code **/
% }

\subsection{Control}
API: fselect(cond, stream1, stream2)
% C Constructs: if(){}  & conditional a?b:
...
\subsection{Streams}
% s_float a; - a stream of floats; can use array access on streams
% a[1] ⇒ stream.offset(s, 1)
% how to handle offset expressions?

% float b; - the value of the stream or a runtime contant;
% b = *a or b = a[0] ---> b takes the value of the current stream element.
...
% via Pragmas: #pragma class:kernelopt name:DSPBalance(1)
via API calls:
