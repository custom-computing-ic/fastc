...
\Cref{sec:fast-extensions}) which were required to meet our design
goals.
...
However, unlike in other approaches \XXX{cit. req.} pragmas are not
...
towards our Portability requirement \XXX{cit. req.}.
...
\begin{itemize}
  ...
  \item If a stream is declared within the body of the dataflow
    kernel, it is a read/write stream (\XXX{Sure?})
\end{itemize}

Additionally casting between variable types can be inferred in some
instances by using type coercion. \XXX{More details here}
