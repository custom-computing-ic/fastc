...
(\Cref{fig:aspect-weaving}) these modules can be combined with
...
\begin{comment}
  ...
  \begin{lstlisting}[label=lst:fast-reconf,caption=FAST support for run-time reconfiguration.]
  ...
  aspect shown in \Cref{fig:aspect-reconf}.  The input to the
...
\Cref{fig:aspect-reconf} selects all function calls, and for each
...
aspects (\Cref{sect:asp_it}) which generate a sequence of solutions
...
\Cref{fig:aspect-exploration}. It highlights the feedback capabilities
...
(\Cref{sect:asp_debug}). Separating these concerns simplifies
...
\Cref{fig:aspect-debug} instruments the code to log every change in
