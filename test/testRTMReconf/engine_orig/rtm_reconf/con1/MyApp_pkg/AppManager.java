...
//  FPGA-0    
    KernelBlock k   = addKernel(new MyApp(makeKernelParameters("MyApp")));
...
// create streams between DRAM and kernel
    k.getInput("burst_p").connect(dram2knl0);
    k.getInput("burst_input").connect(dram2knl1);
       
    knl2dram0.connect(k.getOutput("output_p"));
    knl2dram1.connect(k.getOutput("output_pp"));
	}
}
